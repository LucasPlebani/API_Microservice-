...
const { generateDeviceFingerprint } = require("../utils/securityUtils.js");
...
const authMiddleware = async (req, res, next) => {
  try {
    ...
    if (!isValidNonce) {
      return res
        .status(403)
        .json({ message: "Nonce ou preuve de travail invalide" });
    }
    ...
    if (requestFingerprint !== tokenPayload.deviceFingerprint) {
      return res
        .status(403)
        .json({ message: "Empreinte de l'appareil non reconnue" });
    }
